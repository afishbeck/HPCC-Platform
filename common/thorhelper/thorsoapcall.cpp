...
*/
    ...
    void processEspResponse(Url &url, StringBuffer &response, ColumnProvider * meta)
    {
        ...
        Owned<IXMLParse> xmlParser = createXMLParse((const void *)response.str(), (unsigned)response.length(), path.str(), matchCB, options, master->flags&SOAPFusescontents);
    ...
    void processHttpResponse(Url &url, StringBuffer &response, ColumnProvider * meta)
    {
        ...
        if (strieq(master->acceptType.str(), "application/json"))
            xmlParser.setown(createJSONParse((const void *)response.str(), (unsigned)response.length(), path.str(), matchCB, options, master->flags&SOAPFusescontents, true));
        else
            xmlParser.setown(createXMLParse((const void *)response.str(), (unsigned)response.length(), path.str(), matchCB, options, master->flags&SOAPFusescontents));
        while (xmlParser->next());
    }
