...
public:
        ...
        : logctx(_logctx), outputAllocator(_outputAllocator), clientCert(_clientCert), roxieAbortMonitor(_roxieAbortMonitor)
    {
        ...
        if (0==strncmp(hosts, "secret:", 7))
        {
            ...
            StringBuffer secret;
            getSecret(secret, secretName, "url", false);
            if (!secret.length())
                throw MakeStringException(0, "%sCALL HTTP-CONNECT SECRET url required", wscType == STsoap ? "SOAP" : "HTTP");

            UrlListParser urlListParser(secret);
            getSecret(secret.clear(), secretName, "username", false);
            if (strchr(secret, ':'))
                throw MakeStringException(0, "%sCALL HTTP-CONNECT SECRET username contains illegal colon", wscType == STsoap ? "SOAP" : "HTTP");
            if (secret.length())
            {
                secret.append(':');
                getSecret(secret, secretName, "password", false);
            }
            urlListParser.getUrls(urlArray, secret.str());
            getSecret(proxyAddress, secretName, "proxy", false);
            if (secretExists(secretName, "tls.crt"))
            {
                getSecret(secret.clear(), secretName, "tls.key", false);
                if (secret.isEmpty())
                    throw MakeStringException(0, "%sCALL HTTP-CONNECT SECRET provides client certificate but not private key", wscType == STsoap ? "SOAP" : "HTTP");

                clientCert.setown(new ClientCertificate());
                clientCert->privateKey.set(secret);
                getSecret(secret.clear(), secretName, "tls.crt");
                clientCert->certificate.set(secret);
