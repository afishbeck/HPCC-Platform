...
void CRHRollingCache::advance() {
	CRHRollingCacheElem *e =
			(cache.ordinality() == max) ? cache.dequeue() : NULL; //cache full, remove head element
	if (!eos) {
		if (!e)
...
void CRHLimitedCompareHelper::init(unsigned _atmost, IInputBase *_in,
		ICompare * _cmp, ICompare * _limitedcmp) {
	atmost = _atmost;
...
size32_t CSafeSocket::write(const void *buf, size32_t size,
		bool takeOwnership) {
	...
	try {
		if (httpMode) {
			if (!takeOwnership) {
				...
				if (!newbuf)
					throw MakeStringException(THORHELPER_INTERNAL_ERROR,
							"Out of memory in CSafeSocket::write (requesting %d bytes)",
							size);
...
bool CSafeSocket::readBlock(MemoryBuffer &ret, unsigned timeout,
	...
	try {
		...
		if (len & 0x80000000)
			len ^= 0x80000000;
		if (len > maxBlockSize)
			throw MakeStringException(THORHELPER_DATA_ERROR,
					"Maximum block size (%d bytes) exceeded (missing length prefix?)",
					maxBlockSize);
		if (len) {
			unsigned bytesRead;
...
bool CSafeSocket::readBlock(StringBuffer &ret, unsigned timeout,
		HttpHelper *pHttpHelper, bool &continuationNeeded, bool &isStatus,
		unsigned maxBlockSize) {
...
void CSafeSocket::flush() {
	if (httpMode) {
		...
		header.append("Content-Type: ").append(
				mlFmt == MarkupFmt_JSON ? "application/json" : "text/xml").append(
				"\r\n");
		header.append("Content-Length: ").append(length).append("\r\n\r\n");

		CriticalBlock c(crit); // should not be anyone writing but better to be safe
		if (traceLevel > 5)
			DBGLOG("Writing HTTP header length %d to HTTP socket",
					header.length());
		sock->write(header.str(), header.length());
		sent += header.length();
		if (traceLevel > 5)
			DBGLOG("Writing content head length %d to HTTP socket",
					contentHead.length());
		...
		if (traceLevel > 5)
			DBGLOG("Writing content tail length %d to HTTP socket",
					contentTail.length());
...
void CSafeSocket::sendException(const char *source, unsigned code,
		const char *message, bool isBlocked, const IContextLogger &logctx) {
	try {
		FlushingStringBuffer response(this, isBlocked, MarkupFmt_XML, false,
				httpMode, logctx);
...
FlushingStringBuffer::FlushingStringBuffer(SafeSocket *_sock, bool _isBlocked,
		TextMarkupFormat _mlFmt, bool _isRaw, bool _isHttp,
		const IContextLogger &_logctx) :
		sock(_sock), isBlocked(_isBlocked), mlFmt(_mlFmt), isRaw(_isRaw), isHttp(
				_isHttp), logctx(_logctx) {
	sequenceNumber = 0;
	rowCount = 0;
	isSoap = false;
	isEmpty = true;
...
void FlushingStringBuffer::append(unsigned len, const char *data) {
	try {
		...
		logctx.logOperatorException(E, __FILE__, __LINE__,
				"FlushingStringBuffer::append");
		throw;
	}
}
...
void FlushingStringBuffer::encodeXML(const char *x, unsigned flags,
		unsigned len, bool utf8) {
	StringBuffer t;
...
void FlushingStringBuffer::flush(bool closing) {
	...
	if (isHttp) {
		...
		if (logctx.queryTraceLevel() > 1) {
			if (isBlocked)
				logctx.CTXLOG("Sending reply: Sending blocked %s data",
						getFormatDescription());
			else
#ifdef _DEBUG
				logctx.CTXLOG("Sending reply length %d: %.1024s", (unsigned) (s.length() - sizeof(size32_t)), s.str()+sizeof(size32_t));
#else
				logctx.CTXLOG("Sending reply length %d: %.40s",
						(unsigned) (s.length() - sizeof(size32_t)),
						s.str() + sizeof(size32_t));
#endif
		}
...
void FlushingStringBuffer::startDataset(const char *elementName,
		const char *resultName, unsigned sequence, bool _extend) {
	CriticalBlock b(crit);
	extend = _extend;
	if (isEmpty || !extend) {
		...
		if (!isBlocked) {
			if (mlFmt == MarkupFmt_XML) {
				...
				if (isSoap && (resultName || (sequence != (unsigned) -1))) {
					s.append(" xmlns=\'urn:hpccsystems:ecl:").appendLower(
							queryName.length(), queryName.sget()).append(
							":result:");
...
void FlushingStringBuffer::startScalar(const char *resultName,
		unsigned sequence) {
	CriticalBlock b(crit);
	assertex(!s.length());
...
void FlushingJsonBuffer::encodeXML(const char *x, unsigned flags, unsigned len,
		bool utf8) {
	StringBuffer t;
	::encodeJSON(t, x);
	append(t.length(), t.str());
...
void FlushingJsonBuffer::startDataset(const char *elementName,
		const char *resultName, unsigned sequence, bool _extend) {
	CriticalBlock b(crit);
	extend = _extend;
	if (isEmpty || !extend) {
...
void FlushingJsonBuffer::startScalar(const char *resultName,
		unsigned sequence) {
	CriticalBlock b(crit);
	assertex(!s.length());
...
ClusterWriteHandler::ClusterWriteHandler(char const * _logicalName,
		char const * _activityType) :
		logicalName(_logicalName), activityType(_activityType) {
	makePhysicalPartName(logicalName.get(), 1, 1, physicalName, false);
	splitFilename(physicalName, &physicalDir, &physicalDir, &physicalBase,
			&physicalBase);
}
	...
	if (group->isMember()) {
		if (localCluster)
			throw MakeStringException(0,
					"Cluster %s occupies node already specified while writing file %s",
					cluster, logicalName.get());
...
void ClusterWriteHandler::getLocalPhysicalFilename(StringAttr & out) const {
	if (localCluster.get())
		...
		getTempFilename(out);
	PROGLOG("%s(CLUSTER) for logical filename %s writing to local file %s",
			activityType.get(), logicalName.get(), out.get());
}

void ClusterWriteHandler::splitPhysicalFilename(StringBuffer & dir,
		StringBuffer & base) const {
...
void ClusterWriteHandler::copyPhysical(IFile * source, bool noCopy) const {
	...
	ForEachItemIn(idx, remoteNodes)
	{
		...
		PROGLOG("%s(CLUSTER) for logical filename %s copying %s to %s",
				activityType.get(), logicalName.get(), source->queryFilename(),
				target->queryFilename());
		if (noCopy) {
...
void ClusterWriteHandler::setDescriptorParts(IFileDescriptor * desc,
		char const * basename, IPropertyTree * attrs) const {
	...
	ForEachItemIn(idx, remoteNodes)
		desc->addCluster(remoteClusters.item(idx), &remoteNodes.item(idx),
				partmap);
	if (attrs) {
...
void ClusterWriteHandler::finish(IFile * file) const {
	if (!localCluster.get()) {
		PROGLOG(
				"%s(CLUSTER) for logical filename %s removing temporary file %s",
				activityType.get(), logicalName.get(), file->queryFilename());
		file->remove();
	}
}
...
class COrderedOutputSerializer: implements IOrderedOutputSerializer,
		public CInterface {
	...
	public:
		...
		size32_t printf(const char *format, va_list args) {
			if (closed)
				throw MakeStringException(0,
						"Attempting to append to previously closed result in COrderedResult::printf");
		...
		size32_t fwrite(const void * data, size32_t size, size32_t count) {
			if (closed)
				throw MakeStringException(0,
						"Attempting to append to previously closed result in COrderedResult::fwrite");
		...
		void close(bool nl) {
			if (closed)
				throw MakeStringException(0,
						"Attempting to reclose result in COrderedResult::close");
			if (nl)
				sb.append('\n');
			closed = true;
		}
...
public:
	IMPLEMENT_IINTERFACE
	;
	COrderedOutputSerializer(FILE* _outFile) :
			lastSeqFlushed(-1), outFile(_outFile) {
	}
...
StringBuffer & mangleHelperFileName(StringBuffer & out, const char * in,
		const char * wuid, unsigned int flags) {
	out = in;
...
StringBuffer & expandLogicalFilename(StringBuffer & logicalName,
		const char * fname, IConstWorkUnit * wu, bool resolveLocally) {
	if (fname[0] == '~')
