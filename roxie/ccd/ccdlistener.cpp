...
public:
    ...
    void onException(IException *E)
    {
        ...
        DBGLOG("%s", error.str());
        if (httpHelper.queryPTreeFormat()==MarkupFmt_JSON)
    ...
    void Do(unsigned idx)
    {
        try
        {
            ...
            Owned<IRoxieServerContext> ctx = f->createContext(&request, client, httpHelper.queryPTreeFormat(), false, false, httpHelper, true, logctx, xmlReadFlags);
            ctx->process();
            ctx->flush(idx);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (isHTTP)
                            client->setHttpMode(queryName, isRequestArray, httpHelper.queryPTreeFormat());
                        if (queryFactory)
                        {
