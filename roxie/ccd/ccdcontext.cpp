...
protected:
    ...
    void initDebugMode(bool breakAtStart, const char *debugUID)
    {
        ...
        debugContext.setown(new CRoxieServerDebugContext(this, logctx, factory->cloneQueryXGMML(), *protocol->querySafeSocket()));
...
public:
    ...
    CRoxieServerContext(IPropertyTree *_context, IHpccProtocol *_protocol, const IQueryFactory *_factory, unsigned flags, const ContextLogger &_logctx, PTreeReaderOptions _xmlReadFlags, const char *_querySetName)
        : CRoxieContextBase(_factory, _logctx), serverQueryFactory(_factory), querySetName(_querySetName), protocol(_protocol)
    {
        init();
    ...
    virtual IHpccProtocol *queryProtocol()
    {
        return protocol;
    }
    ...
    virtual void setResultBool(const char *name, unsigned sequence, bool value)
    {
        ...
        {
            protocol->setResultBool(name, sequence, value);
        }
    ...
    virtual void setResultData(const char *name, unsigned sequence, int len, const void * data)
    {
        ...
        {
            protocol->setResultData(name, sequence, len, data);
        }
        if (workUnit)
        {
            try
    ...
    virtual void setResultRaw(const char *name, unsigned sequence, int len, const void * data)
    {
        ...
        {
            protocol->setResultRaw(name, sequence, len, data);
        }
    ...
    virtual void setResultSet(const char *name, unsigned sequence, bool isAll, size32_t len, const void * data, ISetToXmlTransformer * transformer)
    {
        ...
        {
            protocol->setResultSet(name, sequence, isAll, len, data, transformer);
        }
    ...
    virtual void setResultDecimal(const char *name, unsigned sequence, int len, int precision, bool isSigned, const void *val)
    {
        ...
        {
            protocol->setResultDecimal(name, sequence, len, precision, isSigned, val);
        }
        if (workUnit)
        {
            try
    ...
    virtual void setResultInt(const char *name, unsigned sequence, __int64 value, unsigned size)
    {
        ...
        {
            protocol->setResultInt(name, sequence, value, size);
        }
        if (workUnit)
        {
            try
    ...
    virtual void setResultUInt(const char *name, unsigned sequence, unsigned __int64 value, unsigned size)
    {
        ...
        {
            protocol->setResultUInt(name, sequence, value, size);
        }
    ...
    virtual void setResultReal(const char *name, unsigned sequence, double value)
    {
        ...
        {
            protocol->setResultReal(name, sequence, value);
        }
        if (workUnit)
        {
            try
    ...
    virtual void setResultString(const char *name, unsigned sequence, int len, const char * str)
    {
        ...
        {
            protocol->setResultString(name, sequence, len, str);
        }
        if (workUnit)
        {
            try
    ...
    virtual void setResultUnicode(const char *name, unsigned sequence, int len, UChar const * str)
    {
        ...
        {
            protocol->setResultUnicode(name, sequence, len, str);
        }
        if (workUnit)
        {
            try
    ...
    virtual IWorkUnitRowReader *createStreamedRawRowReader(IEngineRowAllocator *rowAllocator, bool isGrouped, const char *id)
    {
        return new StreamedRawDataReader(this, rowAllocator, isGrouped, logctx, *protocol->querySafeSocket(), id); //adf ???
    }
...
public:
    CSoapRoxieServerContext(IPropertyTree *_context, IHpccProtocol *_protocol, const IQueryFactory *_factory, unsigned flags, const ContextLogger &_logctx, PTreeReaderOptions xmlReadFlags, const char *_querySetName)
...
IRoxieServerContext *createRoxieServerContext(IPropertyTree *context, IHpccProtocol *protocol, const IQueryFactory *factory, unsigned flags, const ContextLogger &_logctx, PTreeReaderOptions readFlags, const char *querySetName)
{
