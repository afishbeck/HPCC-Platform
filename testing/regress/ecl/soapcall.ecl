...
output(SORT(SOAPCALL(targetURL,'soapbase', { string unkname := 'FRED' }, dataset(ServiceOutRecord), LOG('simple'), HTTPHEADER('RELX-Global-Id','12345678900'), HTTPHEADER('RELX-Caller-Id','1111')),record));

// double query->dataset form
output(SORT(SOAPCALL(doubleTargetURL,'soapbase', { string unkname := 'FRED' }, dataset(ServiceOutRecord), HTTPHEADER('RELX-Global-Id','12345678900'), HTTPHEADER('RELX-Caller-Id','2222')),record));

// simple dataset->dataset form
output(sort(SOAPCALL(d, targetURL,'soapbase', { unkname }, DATASET(ServiceOutRecord), HTTPHEADER('RELX-Global-Id','12345678900'), HTTPHEADER('RELX-Caller-Id','3333')),record));
...
output(SORT(SOAPCALL(targetURL,'soapbase', { string unkname := 'FAIL' }, dataset(ServiceOutRecord),onFail(doError2),RETRY(0), LOG(MIN), HTTPHEADER('RELX-Global-Id','12345678900'), HTTPHEADER('RELX-Caller-Id','4444')),record));
