    ...
    jfieldID getFieldId(const char* fieldname, const char *sig, const char *expected, StringBuffer *className=NULL)
    {
        ...
        else
        {
            ...
            if (className)
            {
                ...
                checkException();
                jmethodID getFieldType = JNIenv->GetMethodID(fieldClass, "getType", "()Ljava/lang/Class;" );
                checkException();
                jclass fieldType = (jclass)JNIenv->CallObjectMethod(reflectedField, getFieldType);
                checkException();
                jmethodID getFieldTypeName = JNIenv->GetStaticMethodID(fieldClass, "getTypeName", "(Ljava/lang/Class;)Ljava/lang/String;" );
                checkException();
                jstring fieldDescr = (jstring)JNIenv->CallStaticObjectMethod(fieldClass, getFieldTypeName, fieldType);
                checkException();
                className->append(JNIenv->GetStringUTFChars(fieldDescr, NULL));
            }
        }
        if (!fieldId && expected)
...
inline void extractFunctionInfo(const char *text, StringBuffer &classname, StringBuffer &methodname, StringBuffer &fullSig, StringBuffer &javaSig, StringBuffer &retType)
{
    ...
    if (!signature)
        throw MakeStringException(MSGAUD_user, 0, "javaembed: Invalid import name %s - Expected classname.methodname:signature", text);
    classname.append(funcname-text, text);
    funcname++;  // skip the '.'
    methodname.append(signature-funcname, funcname);
    signature++; // skip the ':'
    ...
    while (*finger)
    {
        if (*finger == '<')
        {
            ...
            {
                javaSig.append("Ljava/util/Iterator;");
...
public:
    ...
    : JNIenv(_JNIenv), obj(_obj), writer(_writer), esdl(_esdl), esdlService(_esdlService), reqType(_reqType)
    {
        ...
        langObjectClass = (jclass) JNIenv->NewGlobalRef(JNIenv->FindClass("java/lang/Object"));
...
public:
    ...
    inline IEsdlDefStruct *queryCurrentEsdlStruct()
    {
        if (!defStack.length() || !defStack.tos().type)
            return NULL;
        return dynamic_cast<IEsdlDefStruct*>(defStack.tos().type.get());
    }
    inline jobject getObject()
    {
        if (!defStack.length())
    ...
    virtual void outputBeginNested(const char *fieldname, bool nestChildren)
    {
        ...
        pushDefStackEntry(JNIenv, getCurJavaClass(), getCurJavaObject(), javaPackage, fieldname, defType);
    }
...
public:
    ...
    public:
        ...
        void setClass(jclass localClass)
        {
        }
        void setObject(jobject localObject)
        {
        }
    public:
        StringAttr name;
    ...
    void pushDefStackEntry(JNIEnv *JNIenv, jclass parentClass, jobject parentObject, const char *package, const char *fieldname, IEsdlDefObject *defObj)
    {
        Owned<DefStackEntry> entry = new DefStackEntry(fieldname, defObj);
        JNIenv->ExceptionClear();
        if (defObj)
        {
            VStringBuffer javaClassName("%s/%s", package, defObj->queryName());
            jclass localClass = JNIenv->FindClass(javaClassName);
            if (localClass)
            {
                entry->Class = (jclass) JNIenv->NewGlobalRef(localClass);
                ...
                if (localObj)
                {
                    ...
                    if (parentClass)
                    {
                        ...
                        entry->fieldId = JNIenv->GetFieldID(parentClass, fieldname, javaSig);
                        if (parentObject && entry->fieldId)
                            JNIenv->SetObjectField(parentObject, entry->fieldId, entry->obj);
                    }
                }
            }
        }
...
public:
    ...
    virtual bool checkFunctionExists(const char *function)
    {
        StringBuffer classname;
        StringBuffer methodname;
        StringBuffer javaSig;
        StringBuffer fullSig;
        StringBuffer retType;
        extractFunctionInfo(function, classname, methodname, fullSig, javaSig, retType);

        return (sharedCtx->JNIenv->GetMethodID(Class, methodname, javaSig)!=0);
    }
    ...
    void checkException()
    {
        javaembed::checkException(sharedCtx->JNIenv);
    }
