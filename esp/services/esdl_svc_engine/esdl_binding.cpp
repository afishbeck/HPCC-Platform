...
void EsdlServiceImpl::handleTransformError(bool &serviceError, MapStringTo<StringAttr, const char *> &methodErrors, IException *e, const char *tag, const char *service, const char *method)
{
    VStringBuffer msg("Encountered error while fetching '%s' transforms for service '%s'", tag, service);
...
void EsdlServiceImpl::addTransforms(IEsdlTransformMap *tfm, IPropertyTree *cfgParent, const char *tag, const char *service, const char *method)
{
    try
    {
        tfm->addTransformSet(method, cfgParent, "xsdl", tag);
    }
    catch (IException *e)
    {
        handleTransformError(m_serviceLevelCrtFail, m_requestTransformErrors, e, tag, service, method);
        e->Release();
    }
    catch (...)
    {
        handleTransformError(m_serviceLevelCrtFail, m_requestTransformErrors, nullptr, tag, service, method);
    }
}

void EsdlServiceImpl::addServiceTransforms(IEsdlTransformMap *tfm, IPropertyTree *cfgParent, const char *tag, const char *service)
{
    addTransforms(tfm, cfgParent, tag, service, "");
}
...
void EsdlServiceImpl::configureTargets(IPropertyTree *cfg, const char *service)
{
    ...
    if (target_cfg)
    {
        addServiceTransforms(m_requestTransforms, target_cfg, "CustomRequestTransform", service);
        ...
        ForEach(*iter)
        {
            ...
            m_requestTransforms->remove(method);

            addTransforms(m_requestTransforms, &methodCfg, "CustomRequestTransform", service, method);
...
{
    IEsdlTransformSet *serviceTfs = m_requestTransforms->query("");
    IEsdlTransformSet *methodTfs = m_requestTransforms->query(mthdef.queryMethodName());
    if (!serviceTfs && !methodTfs)
        return nullptr;
                                        ...
                                        StringBuffer &reqProcessed)
{
    ...
    {
        IEsdlTransformSet *serviceTfs = m_requestTransforms->query("");
        IEsdlTransformSet *methodTfs = m_requestTransforms->query(mthName);
        ...
        if (serviceTfs || methodTfs) //slightly redundant, but less fragile to future changes to check again here
            processServiceAndMethodTransforms(scriptContext, {serviceTfs, methodTfs}, ESDLScriptCtxSection_ESDLRequest, ESDLScriptCtxSection_FinalRequest);
        ...
        else
        {
            ...
            else
            {
                try
                {
                    ...
                    m_pESDLService->handleResultLogging(context, scriptContext, tgtctx.get(), req_pt.get(), soapmsg.str(), origResp.str(), out.str(), logdata.str());
                                       ...
                                       CHttpResponse* response)
{
    ...
    try
    {
        ...
        if (getSoapMethodInfo(in, reqname, ns))
        {
            ...
            m_pESDLService->handleResultLogging(*ctx, scriptContext, tgtctx.get(), pt, soapmsg.str(), origResp.str(), out.str(), logdata.str());
...
void EsdlBindingImpl::handleJSONPost(CHttpRequest *request, CHttpResponse *response)
{
    ...
    try
    {
        ...
        m_pESDLService->handleResultLogging(*ctx, scriptContext, tgtctx.get(), reqTree, soapmsg.str(), origResp.str(), jsonresp.str(), logdata.str());
