...
int EsdlBindingImpl::getMethodProperty(IEspContext &context, const char *serv, const char *method, StringBuffer &page, const char *propname, const char *dfault)
{
    ...
    if (!m_esdl)
    {
        ESPLOG(LogMax,"EsdlBindingImpl::getMethodDescription - ESDL definition for service not loaded");
        return 0;
    }
    ...
    if (!srv)
    {
        ESPLOG(LogMax,"EsdlBindingImpl::getMethodDescription - service (%s) not found", serv);
        return 0;
    }
    ...
    if (!mth)
    {
        ESPLOG(LogMax,"EsdlBindingImpl::getMethodDescription - service (%s) method (%s) not found", serv, method);
        return 0;
    }
