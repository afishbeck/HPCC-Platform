...
EsdlBindingImpl::EsdlBindingImpl(IPropertyTree* cfg, IPropertyTree* esdlBundle, const char *binding,  const char *process) : CHttpSoapBinding(cfg, binding, process)
{
    ...
    {
        if (esdlBundle)
            m_esdlBndCfg.set(esdlBundle->queryPropTree("Binding"));
        else
...
bool EsdlBindingImpl::loadLocalDefinitions(IPropertyTree *esdlBundle, const char * espServiceName, Owned<IEsdlDefinition>& esdl, IPropertyTree * esdl_binding, StringBuffer & loadedServiceName)
{
    if (!esdl || !esdl_binding || !esdlBundle)
        return false;
    ...
    if (esdl_binding_definition)
    {
        try
        {
            ...
            if (esdlBundle)
            {
                esdlXML.set(esdlBundle->queryProp("Definitions"));
                if (esdlXML.isEmpty())
...
void EsdlBindingImpl::addService(IPropertyTree *esdlBundle, const char * name,
                                 const char * host,
                                 unsigned short port,
                                 IEspService & service)
{
    ...
    if ( m_pESDLService)
    {
        if (m_esdlBndCfg)
        {
            if (m_esdl)
            {
                ...
                bool loaded = false;
                if (esdlBundle)
                    loaded = loadLocalDefinitions(esdlBundle, name, m_esdl, m_esdlBndCfg, loadedservicename);
                else
