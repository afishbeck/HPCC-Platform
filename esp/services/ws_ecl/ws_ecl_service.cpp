...
int CWsEclBinding::submitWsEclWorkunit(IEspContext & context, WsEclWuInfo &wsinfo, IPropertyTree *reqTree, StringBuffer &out, unsigned flags, CHttpRequest *httpreq, TextMarkupFormat fmt, const char *viewname, const char *xsltname)
{
    ...
    if (httpreq)
    {
        StringBuffer globalId;
        httpreq->getHeader("RELX-Global-Id", globalId);
        StringBuffer callerId;
        httpreq->getHeader("RELX-Caller-Id", callerId);
        if (globalId.length())
        {
    ...
    try
    {
        ...
        if (httpreq)
        {
            ...
            if (globalId.length())
            {
                ...
                headers->setProp("RELX-Global-Id", globalId);

                SocketEndpoint ep;
                StringBuffer localId;
                appendLocalId(localId, httpreq->getSocket()->getEndpoint(ep), 0);
                if (localId.length())
                    headers->setProp("RELX-Caller-Id", globalId);
