...
public abstract class <xsl:value-of select="@name"/>ServiceBase
{
        <xsl:for-each select="EsdlMethod">
    public abstract <xsl:value-of select="@response_type"/><xsl:text> </xsl:text><xsl:value-of select="@name"/>(EsdlContext context, <xsl:value-of select="@request_type"/> request);
        </xsl:for-each>
}
    </xsl:template>
