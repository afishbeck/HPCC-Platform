    ...
    virtual void loadMap(const char *tagname)
    {
        ...
        while (eventType!=YAML_MAPPING_END_EVENT)
        {
            ...
            case YAML_SCALAR_EVENT:
                ...
                {
                    iEvent->beginNode(name, parser.offset);
                    iEvent->endNode(name, event.data.scalar.length, (const void *) event.data.scalar.value, false, parser.offset);
                }
                break;
            case YAML_ALIAS_EVENT: //reference to an anchor, ignore for now
                iEvent->beginNode(name, parser.offset);
                iEvent->endNode(name, 0, nullptr, false, parser.offset);
                break;
