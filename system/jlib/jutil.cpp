...
public:
    ...
    bool requestSecret(CVaultKind &rkind, StringBuffer &content, const char *secret, const char *version)
    {
        if (isEmptyString(secret))
            return false;
        if (getCachedSecret(rkind, content, secret, version))
            return true;
...
static void addCachedSecret(const char *name, IPropertyTree *secret)
{
...
{
    Owned<IPropertyTree> tree = getCachedSecret(name);
    if (tree)
...
extern jlib_decl IPropertyTree *getVaultSecret(const char *category, const char *vaultId, const char * name, const char *version)
{
    ...
    Owned<IPropertyTree> tree = createPTreeFromJSONString(json);
    if (!tree)
        return nullptr;
    switch (kind)
    {
